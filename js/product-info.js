...
function showComments(comments) {
    ...
    for (let i = 0; i < comments.length; i++) {
        ...
        <div class="d-flex justify-content-between gap-2">
            ...
            <small class="text-muted" style="font-size: 12px;">${changeDayFormat(new Date(comment.dateTime))}</small>
        </div>
...
document.getElementById("commentBtn").addEventListener("click", function () {
    ...
    if (!commentArray || !commentData) {
        ...
        if (comment.length < 1) {
            document.getElementById("comment-alert").innerHTML = `<strong>Tiene que ingresar un comentario para poder enviarlo</strong>`
